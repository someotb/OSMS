...
S_in = 4 # лощадь торговых и бизнес центров, где требуется спроектировать сеть на базе микро- и фемтосот (км**2)
...
T_C = -50 # в цельсиях
T_K = T_C + 273.15 # в кельвинах

temp_correction_dB = 10 * np.log10(T_K / 290.0) # 290 К - стандартная температура 17 °C, на ней основано значение -174 дБм/Гц

N_BS_test = -174 + 10 * np.log10(BW_UL) + NF_BS + temp_correction_dB  # дБм
N_UE_test = -174 + 10 * np.log10(BW_DL) + NF_UE + temp_correction_dB  # дБм
...
for i in range(0, len(d)):
    ...
    pl = A + B * np.log10(f_MHz) - 13.82 * np.log10(hBS) - a + s * np.log10(d_km) + Lclutter
    PL_COST.append(pl)
...
L2 = -16.9 - 10.0 * np.log10(w) + 10.0 * np.log10(f_MHz) + 20.0 * np.log10(delta_h_building_mobile) + phi_coef
